...
class ProposalModule(nn.Module):
  ...
  def forward(self, features, pos_anchor_coord=None, \
    ...
    network_output = self.pred_layer(features)  # shape: (B,6*A,H,W) 

    anchor_data  = network_output.view(B,A,6,H,W)    # shape: (B,A,6,H,W) 
    conf_scores  = anchor_data[:,:,:2,:,:]           # shape: (B,A,2,H,W)
    offsets      = anchor_data[:,:,2:,:,:]           # shape: (B,A,4,H,W)
    ...
    if mode == 'train':
      ...
      proposals = proposals.squeeze()                                       # shape: (M, 4)
      
      pos          = self._extract_anchor_data(conf_scores, pos_anchor_idx)    # shape: (M, 2)
      neg          = self._extract_anchor_data(conf_scores, neg_anchor_idx)    # shape: (M, 2)
      conf_scores  = torch.cat((pos, neg), dim=0)                              # shape: (2*M, 2)
...
class RPN(nn.Module):
  ...
  def forward(self, images, bboxes, output_mode='loss'):
    ...
    # 2c. anchors per img (to be returned)
    ...
    pos_anchor_idx, neg_anchor_idx, GT_conf_scores, GT_offsets, GT_class, \
      pos_anchor_coord, neg_anchor_coord = ReferenceOnActivatedAnchors(anc_list, bboxes, grid_list, iou_mat, neg_thresh=0.2, method='YOLO')
    
    # 4. Compute conf_scores, offsets, proposals through the prediction network
    conf_scores, offsets, proposals = self.prop_module(features, pos_anchor_coord, pos_anchor_idx, neg_anchor_idx)

    # 5. Compute regression losses
  ...
  def inference(self, images, thresh=0.5, nms_thresh=0.7, mode='RPN'):
    ...
    with torch.no_grad():
      ...
      conf_scores, offsets = self.prop_module(features)     # shapes : (B,A,2,H,W) , (B,A,4,H,W)
      # 3b. Calculate confidence probabilities
      conf_probs = torch.sigmoid(conf_scores[:,:,0,:,:]).squeeze()  # shape: (B,A,H,W)
      # 3c. Generate proposals
      ...
      conf_probs = conf_probs.reshape(B,-1)                 # shape: (B,A*H*W)
      ...
      for b in range(B):
        ...
        mask_conf = conf_probs[b] > thresh        # shape: (A*H*W,)
        proposals_b  = proposals[b,mask_conf,:]   # shape: (**,4)
        conf_probs_b = conf_probs[b,mask_conf]    # shape: (**,)
        
        # 4b. Filter by nms
        mask_nms = torchvision.ops.nms(proposals_b, conf_probs_b, iou_threshold=nms_thresh)  # shape: (**,1)
        proposals_b  = proposals_b[mask_nms]                  # shape: (*,4)
        conf_probs_b = conf_probs_b[mask_nms].unsqueeze(1)    # shape: (*,1)
        
        # 4c. Append the tensors to final lists
        final_proposals.append(proposals_b)
        final_conf_probs.append(conf_probs_b)
...
class TwoStageDetector(nn.Module):
  ...
  def forward(self, images, bboxes):
    ...
    # 1. Get rpn_loss and other outputs from RPN (first stage)
    rpn_loss, conf_scores, proposals, features, GT_class, pos_anchor_idx, anc_per_img = \
      self.rpn(images, bboxes, output_mode='all')
    # shapes: proposals - (M,4) , pos_anchor_idx - (M,) , GT_class - (M,) 
    # shapes: features - (B,Channels,H,W)
    
    # 2a. Prepare boxes for torchviosion.ops.roi_align
    batch_index = (pos_anchor_idx // anc_per_img).unsqueeze(1).to(proposals.dtype)
    boxes = torch.cat((batch_index,proposals),dim=1)  # shape: (M,5)
    # 2b. Do RoI align on the boxes
    roi_aligned = torchvision.ops.roi_align(features, boxes, (self.roi_output_w, self.roi_output_h))  
    # shape: roi_aligned : (M,Channels,roi_output_w,roi_output_h)
    # 2c. meanpool in the spatial dimension
    # roi_meanpooled = torch.mean(roi_aligned,dim=(2,3))
    roi_meanpooled = nn.functional.avg_pool2d(roi_aligned,(self.roi_output_w, self.roi_output_h)).squeeze()
    # shape: roi_meanpooled : (M,Channels)
    
    # 3. Compute class scores using the cls_layer (second stage)
    class_score = self.cls_layer(roi_meanpooled)   # shape: (M,C)
    
    # 4. Calculate classification loss
    cls_loss   = nn.functional.cross_entropy(class_score, GT_class, reduction='sum') * 1. / images.shape[0]
  ...
  def inference(self, images, thresh=0.5, nms_thresh=0.7):
    ...
    with torch.no_grad():
      # 1. Get proposals and confidence probabilites using the RPN (first stage)
      final_proposals, final_conf_probs, features = self.rpn.inference(images, thresh=thresh, nms_thresh=nms_thresh, mode='FasterRCNN')
      # final_proposals   - list of B tensors of shape (*,4)
      # final_conf_propbs - list of tensors of shape (*,1)
      # features          - shape: (B,Channels,H,W)
      
      # 2. Pass proposals through RoI align and do mean-pooling over spatial dimension
      roi_aligned = torchvision.ops.roi_align(features, final_proposals, (self.roi_output_w, self.roi_output_h))  
      # shape: roi_aligned : (M,Channels,roi_output_w,roi_output_h)
      # roi_meanpooled = torch.mean(roi_aligned,dim=(2,3))
      roi_meanpooled = nn.functional.avg_pool2d(roi_aligned,(self.roi_output_w, self.roi_output_h)).squeeze()
      # shape: roi_meanpooled : (M,Channels)

      # 3a. Compute class scores using the cls_layer (second stage)
      class_score = self.cls_layer(roi_meanpooled)   # shape: (M,C)
      # 3b. Find predicted class
      pred_class = torch.argmax(class_score, dim=1, keepdim=True).to(torch.int64) # shape: (M,1)

      # 4. Store the predicted classes in the list 
      # (i.e. each list element should contain classes of the bth image)
      # 4a. Find number of classes or proposals per image
      cls_per_img = []
      for proposal in final_proposals:
        cls_per_img.append(proposal.shape[0])
      # 4b. Split the pred_class tensor into a list of tensors
      final_class = torch.split(pred_class, cls_per_img, dim=0)
